...
class FeatureGenerator:
    ...
    def __init__(self, df: pd.DataFrame, spacy_model=None) -> None:
        self.raw = df
    ...
    def _extract_syntatic_features(self):
        ...
        self.output['avg_parse_tree_height'] = self.raw.excerpt.apply(get_average_height)
        self.output['mean_parse_subtrees'] = self.raw.excerpt.apply(get_mean_subtrees)
        self.output['noun_chunks'] = self.raw.excerpt.apply(get_averge_noun_chunks)
        self.output['noun_chunk_size'] = self.raw.excerpt.apply(get_noun_chunks_size)
    ...
    def _extract_pos_tag_features(self):
        ...
        self.output['nouns_per_word'] = self.raw.excerpt.apply(lambda x: get_pos_freq_per_word(x, 'NOUN'))
        self.output['proper_nouns_per_word'] = self.raw.excerpt.apply(lambda x: get_pos_freq_per_word(x, 'PROPN'))
        self.output['pronouns_per_word'] = self.raw.excerpt.apply(lambda x: get_pos_freq_per_word(x, 'PRON'))
        self.output['adj_per_word'] = self.raw.excerpt.apply(lambda x: get_pos_freq_per_word(x, 'ADJ'))
        self.output['adv_per_word'] = self.raw.excerpt.apply(lambda x: get_pos_freq_per_word(x, 'ADV'))
        self.output['verbs_per_word'] = self.raw.excerpt.apply(lambda x: get_pos_freq_per_word(x, 'VERB'))
        self.output['cconj_per_word'] = self.raw.excerpt.apply(lambda x: get_pos_freq_per_word(x, 'CCONJ'))
        self.output['sconj_per_word'] = self.raw.excerpt.apply(lambda x: get_pos_freq_per_word(x, 'SCONJ'))

    def _spacy_vectors(self):
        with self.nlp.disable_pipes():
            vectors = np.array([self.nlp(text).vector for text in self.raw.excerpt])
